...
def Avatar(obj):
    if not obj.image_ch:
        return format_html(
            f"<div style='width:35px;margin:0;color:#fff;padding:0;height:35px'><p style='display: flex;align-items: center;justify-content:center; background-color:blue; width:30px;margin:0;color:#fff;padding:0;height:30px;color:fff !important;border-radius:50%'>{obj.name[0]}</p></div>"
        )
